...
## Cytotoxicity assessment (panel B): preranked GSEA over the MSigDB hallmark
## collection (Liberzon et al. 2015), run with fgsea. All 50 hallmark sets are
## tested in every treatment and reported in Supplementary Table S2; the panel
## displays the sets that constitute the transcriptional response to cytotoxic
## exposure, defined a priori:
FOCAL_SETS <- c("p53 pathway"            = "HALLMARK_P53_PATHWAY",
                "Apoptosis"              = "HALLMARK_APOPTOSIS",
                "Unfolded protein resp." = "HALLMARK_UNFOLDED_PROTEIN_RESPONSE",
                "Reactive oxygen"        = "HALLMARK_REACTIVE_OXYGEN_SPECIES_PATHWAY",
                "G2M checkpoint"         = "HALLMARK_G2M_CHECKPOINT",
                "E2F targets"            = "HALLMARK_E2F_TARGETS")
GSEA_MIN <- 15; GSEA_MAX <- 500     # set size limits
GSEA_SEED <- 20260917
NES_CAP <- 3                        # colour scale limit for the panel
...
bioc <- c("tximport", "DESeq2", "fgsea", "msigdbr")
missing <- cran[!vapply(cran, requireNamespace, logical(1), quietly = TRUE)]
if (length(missing)) install.packages(missing)
for (p in bioc)
  if (!requireNamespace(p, quietly = TRUE))
    stop("Install missing package: ", p, " (fgsea and msigdbr are needed for the cytotoxicity GSEA)")
suppressPackageStartupMessages({
  ...
  library(openxlsx); library(tximport); library(DESeq2); library(fgsea)
})
...
## MSigDB hallmark collection, as gene symbols
get_hallmark <- function() {
  h <- tryCatch(msigdbr::msigdbr(species = "Homo sapiens", collection = "H"),
                error = function(e) tryCatch(msigdbr::msigdbr(species = "Homo sapiens", category = "H"),
                                             error = function(e2) NULL))
  if (is.null(h) || !all(c("gs_name", "gene_symbol") %in% names(h)))
    stop("Could not retrieve the MSigDB hallmark collection with msigdbr")
  split(toupper(h$gene_symbol), h$gs_name)
}
HALLMARK <- get_hallmark()
HALLMARK <- lapply(HALLMARK, unique)
message(sprintf("MSigDB hallmark collection: %d sets", length(HALLMARK)))
missing_focal <- setdiff(FOCAL_SETS, names(HALLMARK))
if (length(missing_focal)) stop("Hallmark set(s) not found: ", paste(missing_focal, collapse = ", "))
...
## gene-level ranking statistic: the DESeq2 Wald statistic of the most
## significant transcript for each gene (signed, and continuous, so no
## significance threshold enters the ranking)
GENES <- bind_rows(lapply(seq_len(nrow(cyto_arms)), function(i) {
  read_csv(cyto_arms$file[i], show_col_types = FALSE,
           col_select = c("log2FoldChange","stat","pvalue","padj","gene_symbol")) %>%
    mutate(gene = toupper(trimws(gene_symbol))) %>%
    filter(!is.na(gene), gene != "", !is.na(padj), !is.na(stat)) %>%
    arrange(padj) %>% distinct(gene, .keep_all = TRUE) %>%
    transmute(line = cyto_arms$line[i], drug = cyto_arms$drug[i], gene,
              lfc = log2FoldChange, stat, pvalue, padj)
}))


# ---- 12. Preranked GSEA over the hallmark collection --------------------------

set.seed(GSEA_SEED)
gsea <- bind_rows(lapply(seq_len(nrow(cyto_arms)), function(i) {
  g <- GENES %>% filter(line == cyto_arms$line[i], drug == cyto_arms$drug[i])
  ranks <- setNames(g$stat, g$gene)
  ranks <- sort(ranks[is.finite(ranks)], decreasing = TRUE)
  res <- suppressWarnings(fgsea::fgseaMultilevel(pathways = HALLMARK, stats = ranks,
                                                 minSize = GSEA_MIN, maxSize = GSEA_MAX))
  as_tibble(res) %>%
    transmute(line = cyto_arms$line[i], drug = cyto_arms$drug[i],
              pathway, size, NES, pval, padj_within_arm = padj,
              leading_edge = vapply(leadingEdge, paste, character(1), collapse = ";"),
              n_leading = lengths(leadingEdge))
})) %>%
  mutate(padj_all = p.adjust(pval, "BH"),          # across all sets and arms
         drug = factor(drug, levels = names(DRUGS)),
         line = factor(line, levels = names(LINES)))

n_tests <- nrow(gsea); n_hit <- sum(gsea$padj_all < 0.05, na.rm = TRUE)
focal <- gsea %>% filter(pathway %in% FOCAL_SETS)
n_focal <- nrow(focal); n_focal_hit <- sum(focal$padj_all < 0.05, na.rm = TRUE)
message(sprintf("\nGSEA: %d of %d hallmark tests significant (FDR < 0.05); %d of %d among the %d cytotoxicity sets",
                n_hit, n_tests, n_focal_hit, n_focal, length(FOCAL_SETS)))
if (n_focal_hit)
  print(as.data.frame(focal %>% filter(padj_all < 0.05) %>%
                        transmute(line, drug, pathway, size, NES = round(NES, 2),
                                  padj = signif(padj_all, 3), n_leading)), row.names = FALSE)

add_table("S2a_cytotoxicity_gsea", "Table S2a",
  "Cytotoxicity assessment: preranked GSEA over the MSigDB hallmark collection",
  paste("Preranked GSEA (fgsea) of every MSigDB hallmark gene set in each cell line and treatment, ranked on the",
        "DESeq2 Wald statistic of the most significant transcript per gene. NES, normalised enrichment score;",
        "padj_within_arm, Benjamini-Hochberg adjustment within an arm; padj_all, adjustment across all sets and",
        "arms; n_leading, genes in the leading edge. Positive NES indicates enrichment among up-regulated genes."),
  gsea %>% select(-leading_edge))

add_table("S2b_cytotoxicity_leading_edge", "Table S2b",
  "Cytotoxicity assessment: leading-edge genes",
  paste("Leading-edge genes for each hallmark set reaching FDR < 0.05 across all tests, with the gene-level",
        "log2 fold-change and adjusted p-value from the corresponding contrast."),
  gsea %>% filter(padj_all < 0.05) %>%
    select(line, drug, pathway, NES, padj_all, leading_edge) %>%
    tidyr::separate_rows(leading_edge, sep = ";") %>%
    rename(gene = leading_edge) %>%
    left_join(GENES %>% select(line, drug, gene, lfc, padj), by = c("line","drug","gene")))

add_table("S2c_cytotoxicity_set_membership", "Table S2c",
  "Cytotoxicity assessment: hallmark sets displayed in the figure",
  "Membership of the MSigDB hallmark sets shown in Supplementary Figure S1B, and whether each gene was quantified.",
  tibble(pathway = rep(FOCAL_SETS, lengths(HALLMARK[FOCAL_SETS])),
         set = rep(names(FOCAL_SETS), lengths(HALLMARK[FOCAL_SETS])),
         gene = unlist(HALLMARK[FOCAL_SETS], use.names = FALSE)) %>%
    mutate(quantified = gene %in% GENES$gene))


# ---- 13. Panel B -----------------------------------------------------------

pd_S <- focal %>%
  mutate(set = factor(names(FOCAL_SETS)[match(pathway, FOCAL_SETS)],
                      levels = rev(names(FOCAL_SETS))),
         arm = factor(as.character(drug), levels = names(DRUGS)),
         hit = !is.na(padj_all) & padj_all < 0.05,
         NES_disp = pmax(pmin(NES, NES_CAP), -NES_CAP))

p_SUM <- ggplot(pd_S, aes(arm, set, fill = NES_disp)) +
  geom_tile(colour = "white", linewidth = 0.4) +
  geom_tile(data = filter(pd_S, hit), fill = NA, colour = "grey10", linewidth = 0.6) +
  geom_text(aes(label = sprintf("%.1f", NES)), size = PT(SZ$annot) / .pt, colour = "grey15") +
  scale_fill_gradient2(low = COL_DOWN, mid = "white", high = COL_UP, midpoint = 0,
                       limits = c(-NES_CAP, NES_CAP), na.value = "grey92",
                       name = "GSEA NES") +
  scale_x_discrete(expand = c(0, 0)) + scale_y_discrete(expand = c(0, 0)) +
  facet_grid(~ line, scales = "free_x", space = "free_x") +
  labs(title = "Hallmark responses to cytotoxic exposure (GSEA)", x = NULL, y = NULL,
       caption = sprintf(paste("Normalised enrichment scores from preranked GSEA of the MSigDB hallmark collection;",
                               "outlined cells reach FDR < 0.05 across all %d tests (%d hallmark sets x %d treatments)."),
                         n_tests, length(HALLMARK), nrow(cyto_arms))) +
  theme_pub +
...
cap_B <- paste0(
  "Transcriptional responses that accompany cytotoxic exposure, assessed by preranked gene set enrichment analysis ",
  "(GSEA) of the MSigDB hallmark collection (Liberzon et al. 2015) in each treatment, ranked on the DESeq2 Wald ",
  "statistic. The sets shown are those that constitute the response to cytotoxic exposure, defined a priori; all 50 ",
  "hallmark sets were tested and are reported in Supplementary Table S2. Colour and numbers give the normalised ",
  "enrichment score, with positive values indicating enrichment among up-regulated genes. Outlined cells reach ",
  sprintf("FDR < 0.05 across all %d tests. Leading-edge genes are given in Supplementary Table S2.", n_tests))
...
legend <- paste0(
  ...
  "(B) Transcriptional responses that accompany cytotoxic exposure, assessed by preranked gene set enrichment analysis (GSEA) of the ",
  "MSigDB hallmark collection (Liberzon et al. 2015) in each treatment, ranked on the DESeq2 Wald statistic. The sets shown are those ",
  "that constitute the response to cytotoxic exposure, defined a priori; all 50 hallmark sets were tested and are reported in ",
  "Supplementary Table S2. Colour and numbers give the normalised enrichment score, positive values indicating enrichment among ",
  sprintf("up-regulated genes. Outlined cells reach FDR < 0.05 across all %d tests. Leading-edge genes are given in Supplementary Table S2. ", n_tests),
